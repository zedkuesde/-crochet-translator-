...
Formulation des actions : `Fais 2 × Maille serrée.` (quantité lisible, **label exact** de la base, sans pluralisation). Un qualificatif s’affiche après le label, en forme canonique (`dans chaque maille` / `dans la maille suivante` / `dans un anneau magique`). Le suffixe « jusqu’à la fin » produit `Répète jusqu’à la fin du rang/tour :`, jamais un nombre de répétitions. Si au moins une action a un qualificatif de position, une note de prudence rappelle que l’application ne calcule ni le nombre de mailles ni la position exacte.

Exemple réel de démarrage d’amigurumi : `Tour 1 : 8 ms dans un anneau magique [8]` devient « Pour le tour 1 : » puis `Fais 8 × Maille serrée dans un anneau magique.`, avec la note de prudence et le total recopié (`8`). La forme collée sans espace avant le crochet (`Tour 1 : 8 ms dans un anneau magique[1]`) est aussi reconnue ; le total affiché est celui écrit dans le patron, jamais un calcul. Le qualificatif n’est admis que sur **une action unique**, hors répétition (`xN`, `(…) N fois`, `, à répéter`, `jusqu’à la fin`) et hors séquence à virgule.
...
9. Coller un patron amigurumi réel et vérifier : `Tour 1 : 8 ms dans un anneau magique [8]` et `Tour 1 : 8 ms dans un cercle magique [8]` sont expliqués (rendu canonique `dans un anneau magique`, note de prudence, total recopié `8`) ; « anneau magique » n’est pas cliquable ; la forme réelle `Tour 1 : 8 ms dans un anneau magique[1]` l’est aussi (total recopié `1`) ; `Tour 3 : (1 ms, 1 aug) 8 fois [24]` et `Tour 11 : 1 ms dans chaque m [48]` restent expliqués ; le `m` de `dans chaque m` n’est pas cliquable ; `Tour 2 : 8 aug[2]` est expliqué (quantité + terme + `[2]` recopié, pas calculé) ; `Tour 2 : aug[2]`, `8 ms dans un anneau magique, 1 aug[3]`, `Tours 6-9 (4 tours) : 1 ms dans chaque m[3]` restent non expliqués (aucune liste partielle). Le total affiché est celui écrit dans le patron, jamais un calcul.
