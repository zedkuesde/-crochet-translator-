...
- `Tour 3 : (1 ms, 1 aug) 8 fois[1]` → « Pour le tour 3 : », `Répète 8 fois :`, deux actions, total recopié `1`.
...
La forme collée sans espace avant le crochet (`Tour 1 : 8 ms dans un anneau magique[1]`) reste reconnue. Le qualificatif n’est admis que sur **une action unique**, hors répétition (`xN`, `(…) N fois`, sandwich, `, à répéter`, `jusqu’à la fin`) et hors séquence à virgule. Un sandwich n’accepte aucun qualificatif.
...
9. Coller un patron amigurumi réel et vérifier : `Tour 1 : 8 ms dans un anneau magique[2]` (rendu canonique `dans un anneau magique`, note de prudence, total recopié `2`) ; « anneau magique » n’est pas cliquable ; `Tour 3 : (1 ms, 1 aug) 8 fois[1]` (une section `Répète 8 fois :`, total recopié `1`) ; `Tour 11 : 1 ms dans chaque m` reste expliqué et le `m` n’est pas cliquable ; `Tour 15 : 7 ms, 1 aug, 4 ms, 1 aug, (7 ms, 1 aug) 2 fois, 1 ms, 1 aug, 7 ms, 1 aug[3]` affiche trois sections titrées et le total recopié `3` ; `Tour 17 : 7 ms, 1 dim, 4 ms, 1 dim, (7 ms, 1 dim) 2 fois, 1 ms, 1 dim, 7 ms, 1 dim` affiche les mêmes trois titres sans ligne de total ; `1 ms, (2 ms, 1 aug) 3 fois, 1 dim[4]` garde trois sections même avec une seule action avant et après ; `Tour 19 : 1 ms, 1 dim, 3 ms, 1 dim, 2 ms ; fil blanc : 1 ms, 1 dim, (3 ms, 1 dim) 2 fois, 2 ms[6]` reste non expliqué (aucune liste partielle). Le texte du patron ne change jamais. Le total affiché est celui écrit dans le patron, jamais un calcul.
